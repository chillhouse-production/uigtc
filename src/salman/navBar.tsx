...
const Navbar = () => {
  ...
  const menuItemsLoggedIn = [
    { id: 'home', icon: iconHome, path: '/' },
    { id: 'toko', icon: iconToko, path: '/' },
    { id: 'keranjang', icon: iconKeranjang, path: '/' },
  ];
    ...
    <>
        ...
        <div className="max-w-[2000px] mx-auto px-4 sm:px-6 lg:px-8 border-l border-r border-[#3d2314]/30 bg-white shadow-md">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2">
              {menuItems.map((item) => (<a key={item.id} href={item.path} className={`p-3 rounded-lg transition-all duration-200 ${activeMenu === item.id ? 'bg-teal-600 shadow-md' : 'hover:bg-gray-100'}`} onMouseEnter={() => setActiveMenu(item.id)} onMouseLeave={() => setActiveMenu('')}><img src={item.icon} alt={item.id} className="h-6 w-6" /></a>))}
              {isLoggedIn ? (
                <div className="relative">
                  <button className={`flex items-center space-x-1 p-3 rounded-lg transition-all duration-200 ${showDropdown ? 'bg-teal-600 shadow-md' : 'hover:bg-gray-100'}`} onClick={() => setShowDropdown(!showDropdown)} onMouseEnter={() => setActiveMenu('profile')} onMouseLeave={() => setActiveMenu('')}><img src={showDropdown ? iconProfilePutih : iconProfileHitam} alt="Profile" className="h-6 w-6" /><svg className={`h-4 w-4 transition-transform duration-200 ${showDropdown ? 'rotate-180 text-white' : 'text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></button>
                ...
                <a href="/" className="bg-teal-600 hover:bg-teal-700 text-white font-semibold py-2 px-6 rounded-lg transition-all duration-200">Login</a>
              )}
            </div>
          </div>
        </div>
      ...
      <div className={`fixed inset-0 z-[200] md:hidden transform transition-transform duration-300 ${showMobileSidebar ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="fixed right-0 top-0 h-full w-64 bg-white shadow-xl">
          ...
          <div className="py-4">
            <a href="/" className="block px-6 py-3 text-gray-700 hover:bg-gray-100 transition-colors font-medium">Home</a>
            {isLoggedIn && (
              <>
                <a href="/" className="block px-6 py-3 text-gray-700 hover:bg-gray-100 transition-colors font-medium">Merchandise</a>
                <a href="/" className="block px-6 py-3 text-gray-700 hover:bg-gray-100 transition-colors font-medium">Cart</a>
              </>
            )}
            {isLoggedIn ? (
              <>
                <div className="border-t my-2" />
                <button onClick={() => setShowMobileProfileDropdown(!showMobileProfileDropdown)} className="w-full flex items-center justify-between px-6 py-3 text-gray-700 hover:bg-gray-100 transition-colors font-medium"><span>Profile</span><svg className={`h-4 w-4 transition-transform duration-200 ${showMobileProfileDropdown ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg></button>
              ...
              <div className="px-6 py-4"><a href="/" className="block w-full text-center bg-teal-600 text-white py-2 px-4 rounded-lg hover:bg-teal-700 transition-colors font-medium">Login</a></div>
            )}
          </div>
        </div>
      </div>
